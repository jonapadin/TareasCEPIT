...
function burbujas(arreglo: number[], cantidad: number) {
    for (let i: number = 0; i < cantidad; i++) {
...
let limiteNumero: number = rls.questionInt("Ingrese la longitud del arreglo: ");
let arregloNumero: number[] = new Array(limiteNumero);
cargar(arregloNumero, limiteNumero, 100);
escribirEnUnaLinea(arregloNumero, limiteNumero);
burbujas(arregloNumero, limiteNumero);
escribirEnUnaLinea(arregloNumero, limiteNumero);